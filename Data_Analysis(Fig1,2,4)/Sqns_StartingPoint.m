% plot sequence accracy by starting point, regardless of orientation
...
for i = 1:size(participants,2)
    ...
    trialnum = stpACtable2(:,7:end);
    data2plot = stpACtable2(:,1:6);
...
%     [H_normal(i,1), pValue_normal(i,1), SWstatistic_normal(i,1)] = swtest(reshape(data2plot,size(data2plot,1)*size(data2plot,2),1),0.05);  % 0- normally distributed
     ...
     % RM-ANOVA
     for kk =1:6
         name_temp{kk} =['pos' num2str(kk)] ;
     end
     t = array2table( data2plot,'VariableName',name_temp);  clear name_temp
     rm = fitrm(t,'pos1-pos6~1');
     tbl_anova{i,1} = ranova(rm);
     cANOVA{i,1} = multcompare(rm,'Time','ComparisonType','bonferroni');
     sig(i,1) = tbl_anova{i,1}.pValue(1);
    ...
    kendallsw(i,1) = chisquare_npara(i,1)/(30*(6-1));
    ...
    errorbar([1:6],data_mean,data_errorbar,'LineWidth',plot_linewidth,'Color',map(i,:));
        % set ticks
    mini_tick = 0.2;
    ...
    if sig_npara(i,1)<0.001
        sigflag = '***';
        marker_fontsize = text_fontsize+2;
    ...
    elseif sig_npara(i,1)<0.05
        ...
        sigflag = 'n.s.';
        marker_fontsize = text_fontsize;
    end
    text(3,min(upper_limit,1)-0.04*(min(upper_limit,1)-lower_limit),sigflag,'FontName','Arial','FontWeight', 'bold','FontSize',marker_fontsize)
    hold off;
...
xlb = xlabel('Starting Point','FontName', 'Arial','FontWeight', 'bold','FontSize',label_fontsize);
