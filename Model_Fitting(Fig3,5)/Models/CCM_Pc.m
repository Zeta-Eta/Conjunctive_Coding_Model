...
% Path-crossings Version
% with Laplace Distribution(Order) & von Mises Distribution(Item)
% with Weight
% with Background Noise
% with Path-crossings(lambda is ralated to the path-crossings number of a sequence)

function [L, ModelParams, EncodingMatrix, RetrievalPMF] = ...
    CCM_Pc(Params, Experiment, dataSet, LF)
...
lambda = abs(Params(setsize + 2:end - 1));% Order Precision Parameter
...
for pattern = 1:patternN
    ...
    Order2Target = exp(- lambda(dataSet.UpathCid(pattern)).*Order);
