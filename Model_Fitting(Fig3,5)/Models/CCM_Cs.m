...
% with Chunk-size(lambda is ralated to the size of each chunk in a sequence)

function [L, ModelParams, EncodingMatrix, RetrievalPMF] = ...
    CCM_Cs(Params, Experiment, dataSet, LF)
...
for pattern = 1:patternN
    ...
    Order2Target = exp(- lambda(dataSet.UckSid(pattern, :)).*Order);
