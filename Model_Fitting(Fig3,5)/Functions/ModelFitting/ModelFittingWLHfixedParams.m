% Model Fitting with the Latter Half Fixed Params

function FittingResults = ...
    ModelFittingWLHfixedParams(trainingSet, testSet, ModelName, ...
    freeParams, fixedParams, Experiment, LossFunction)
...
func = @(params) Model([params, fixedParams], Experiment, trainingSet, LossFunction);
...
options.TolX = 1e-7;
options.Display = 'off';
modelParams = fminsearch(func, freeParams, options);
...
[RSS, ModelParams, EncodingMatrix, RetrievalPMF] =...
    Model([modelParams, fixedParams], Experiment, testSet, 'LSE');
...
FittingResults.freeParams = freeParams;
